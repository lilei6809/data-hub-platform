...
//     * @throws KeycloakUnauthorizedException 当 Service Account 凭证失效时（不可重试）
     ...
     */
    UserId ensureUser(Email email, TemporaryCredentialPolicy credentialPolicy);

    /**
     * 确保用户是 Organization 的成员。
     * 关系已存在时视为成功，不抛异常。
     */
    void ensureOrganizationMembership(OrganizationId organizationId, UserId userId);
     ...
     * 已分配则跳过。
     */
    void ensureUserRealmRole(UserId userId, RealmRoleName realmRoleName);
