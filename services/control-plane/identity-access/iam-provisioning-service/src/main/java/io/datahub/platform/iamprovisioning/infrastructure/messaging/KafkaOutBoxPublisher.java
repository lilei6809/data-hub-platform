...
public class KafkaOutBoxPublisher {
    ...
    private final String instanceId = System.getenv().getOrDefault(
            "HOSTNAME", "unknown-" + UUID.randomUUID().toString().substring(0, 8)
    );
...
//        List<OutBoxEvent> events = repository.findPendingForPublish(BATCH_SIZE);
        ...
        List<OutBoxEvent> events = repository.claimBatch(BATCH_SIZE, instanceId);
    ...
    /**
     * 超时回收：清理崩溃实例遗留的 CLAIMING 事件。
     * 独立于主轮询，以固定频率执行。
     */
    @Scheduled(fixedRate = 60_000)
    public void reclaimStaleEvents(){
    ...
    private void publishSingleEvent(OutBoxEvent event) {
        try {
            ...
            // InterruptedException 后不继续处理剩余事件
            // 剩余的 CLAIMING 事件会被超时回收
            return;
        } catch (Exception e) {
