...
public class EnsureOrganizationMembershipStep implements TenantIamProvisioningStep {
    ...
    public StepExecutionContext ensure(TenantIamDesiredState desired, StepExecutionContext context) {
        ...
        try {
            ...
            OrganizationId organizationId = context.requireOrganizationId(IamProvisioningStep.ENSURE_ORGANIZATION_MEMBERSHIP);
            UserId userId = context.requireUserId(IamProvisioningStep.ENSURE_ORGANIZATION_MEMBERSHIP);
            ...
            throw new IamProvisioningException(
                    IamProvisioningStep.ENSURE_ORGANIZATION_MEMBERSHIP,
                    ex.getFailureCode(),
                    ex.getMessage(),
                    ex.isRetryable(),
                    ex
    ...
    @Override
    public String name() {
        return IamProvisioningStep.ENSURE_ORGANIZATION_MEMBERSHIP.name();
    }
