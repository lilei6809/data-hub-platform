---
id: "94bc3678-bc1a-49eb-a62d-646da8282083"
entity_type: "brief"
entity_id: "092e4d88-b9b5-4137-b557-de7ec1048ff4"
title: "Tenant IAM Onboarding for Multi-Tenant CDP Platform"
status: "draft"
priority: ""
updated_at: "2026-05-23T05:44:42.423341+00:00"
...
本 PRD 定义 Identity & Access Layer（IAL）中第一阶段要交付的 **Tenant IAM Onboarding** 能力，并吸收 `DDD.md` 中已经定稿的授权架构基线。
...
- MVP 实现范围：Tenant IAM Onboarding 的领域模型、幂等 Step Pipeline、状态机、Ports、Fake/In-Memory Adapter 和验证场景。
- 架构基线范围：Authorization BC、Tenant Context 传播、Envoy 静态 RBAC、Spring Security ABAC、authorization-service API 契约、Fail-Closed 策略和后续演进路径。架构基线进入 PRD 作为后续任务拆分依据，但不要求在 MVP 中完整实现。

核心边界必须保持清晰：Keycloak 负责身份签发；Envoy Gateway 负责 JWT 验证、静态 RBAC 和可信 Header 注入；Spring Boot 应用只消费 Envoy 注入的 Header；Authorization BC 负责资源级 ABAC 策略评估。下游服务不得直接信任客户端传入的身份 Header，也不得把 JWT 解析逻辑扩散到业务服务内部。
...
## 7. DDD Bounded Context Baseline

IAL 的 DDD 边界必须显式进入需求，因为后续任务会跨 Tenant Management、Authorization 和 Audit 三个上下文协作。

核心上下文：

- `Tenant Management BC`：租户元数据和成员关系的 Source of Truth。下游不能自行维护租户成员事实副本。
- `Authorization BC`：Core Domain，负责平台角色、自定义角色、策略和 ABAC 决策。它是授权决策的唯一权威执行者。
- `Audit BC`：异步消费授权和租户生命周期事件，当前采用 Conformist 模式跟随上游事件 Schema。

Authorization BC 内部的第一版战术模型：

- `Role` Aggregate：租户自定义角色定义，写少读多。
- `RoleAssignment` Aggregate：用户与角色的绑定关系，是授权热路径。
- `Policy` Aggregate：租户 ABAC 策略载体；一个租户任意时刻只有一个生效 Policy。

服务边界要求：

- `Role`、`RoleAssignment`、`Policy` 不拆成三个微服务，统一收敛到单一 authorization-service 边界。
- 访问频率差异通过缓存解决，而不是通过服务拆分解决。
- Domain 层不能感知 HTTP、JWT、Keycloak SDK、数据库 schema 切换或 Kafka 细节。

## 8. Trusted Tenant Context Propagation

系统必须建立从 JWT 到应用内上下文的可信传播链。

JWT 约束：

- 必须包含 `sub`、`aud`、`exp`、`tenant_id`、`realm_access.roles`。
- `tenant_id` 通过 Keycloak Protocol Mapper 注入，来自用户属性或组织映射后的稳定身份事实。
- JWT 只携带平台预定义角色，例如 `TENANT_ADMIN`、`DATA_ENGINEER`、`VIEWER`。
- JWT 不携带租户自定义角色，因为自定义角色变化频繁、容易造成 JWT 授权滞后和体积膨胀。

Envoy Header 约束：

- Envoy 必须剥除客户端传入的身份相关 Header。
- Envoy 验证 JWT 后注入 `X-Tenant-ID`、`X-User-ID`、`X-Platform-Roles`。
- 下游服务只能信任 Envoy 注入的 Header，不能信任客户端直传 Header。

应用内上下文要求：

- `TenantContextFilter` 将 Header 翻译成 `TenantAwareAuthentication` 和 `TenantContextHolder`。
- `TenantContextHolder` 当前阶段可基于 `ThreadLocal`，但必须在 `finally` 中清理。
- 异步任务必须通过 `TenantAwareExecutor` 捕获并恢复租户上下文和 SecurityContext。

## 9. Authorization Evaluation Contract

系统采用双层授权模型：

- Envoy Gateway 执行基于 JWT Claims 的静态 RBAC，只判断平台角色是否可访问 API 端点。
- Spring Security 和 authorization-service 执行资源级 ABAC，在资源属性可用后做最终授权决策。

ABAC 评估要求：

- `@PreAuthorize` 只做角色预筛，避免无效数据库 IO。
- 资源从数据库取出后，业务服务通过 Thin Client SDK 调用 authorization-service。
- authorization-service 接收完整 `EvaluationContext`，包含 Subject、Resource、Action 和预留 Environment。
- DENY 是正常业务结果，不是系统错误。
- authorization-service 不可用时默认 Fail-Closed，业务服务应返回 503，而不是放行请求。

第一版 deny reason 枚举：

- `NO_MATCHING_POLICY_RULE`
- `SUBJECT_NOT_IN_TENANT`
- `RESOURCE_NOT_IN_TENANT`
- `POLICY_EVALUATION_ERROR`

这些约束重要，因为多租户数据隔离优先级高于可用性。任何 Fail-Open、信任客户端 Header、把自定义角色塞进 JWT、或在 Envoy 热路径做动态数据库查询的方案，都必须视为架构偏离。
...
- 业务服务开发者：需要通过稳定的 Tenant Context 和 Thin Client SDK 调用授权决策，不直接解析 JWT。
- 安全与平台运维人员：需要明确信任边界、失败策略、审计事件和跨租户访问告警语义。
...
1. 客户端携带 Keycloak JWT 请求平台 API。
2. Envoy 剥除客户端身份 Header，验证 JWT 签名、过期时间和受众。
3. Envoy 基于 JWT 平台角色执行静态 RBAC。
4. Envoy 注入 `X-Tenant-ID`、`X-User-ID`、`X-Platform-Roles`。
5. Spring Boot `TenantContextFilter` 构建 `TenantAwareAuthentication` 和 `TenantContextHolder`。
6. Application Service 查询资源并校验资源归属租户。
7. Application Service 调用 authorization-service 做 ABAC 评估。
8. ALLOW 时继续业务逻辑；DENY 时返回访问拒绝；authorization-service 不可用时 Fail-Closed 返回 503。
...
- 授权拒绝日志必须记录 `decisionId` 和枚举型 `denyReason`，避免记录敏感资源属性全文。
...
授权架构基线推荐采用中心化 PDP + 轻量 PEP 结构：

```text
Envoy Gateway
  -> TenantContextFilter
  -> Application Service PEP
  -> AuthorizationThinClient
...
- `TenantContextFilter`：把 Envoy 注入的可信 Header 翻译成 `TenantAwareAuthentication` 和 `TenantContextHolder`。
- `AuthorizationThinClient`：封装业务服务到 authorization-service 的调用、超时、熔断和 Fail-Closed 行为。
- `authorization-service`：ABAC 决策的 PDP，集中执行 `Policy.evaluate()`，业务服务不得复制策略评估逻辑。
- `OutboxPublisher`：授权决策、角色绑定、策略变更等事件必须通过 Outbox Pattern 可靠发布。
...
- `TenantId` 使用稳定 slug，例如 `tenant-acme-corp`。
- `SubjectId` 对应 Keycloak `sub` Claim。
- `RoleName` 使用大写字母和下划线，例如 `REGIONAL_STEWARD`。
- `Permission` 使用 `resource:action` 格式，例如 `datasource:read`。
...
```text
tenant_id
  source: Keycloak Protocol Mapper
  example: tenant-acme-corp

realm_access.roles
  source: Keycloak Realm Role Mapper
  example: TENANT_ADMIN, DATA_ENGINEER
  constraint: platform roles only; custom tenant roles are excluded
```
...
X-Platform-Roles
  source: JWT realm_access.roles
  format: comma-separated values
```

authorization-service 评估接口：
...
- `200 OK`：授权评估完成，`ALLOW` 和 `DENY` 都返回 200。
- `400 Bad Request`：请求体非法，例如 permission 不符合 `resource:action`。
- `401 Unauthorized`：服务间身份凭证缺失或无效。
- `503 Service Unavailable`：authorization-service 内部错误或依赖不可用，调用方必须 Fail-Closed。
...
- JWT 验证必须在 Envoy Gateway 完成，下游 Spring Boot 服务不直接解析客户端 JWT。
- Envoy 必须剥除客户端传入的身份 Header，再注入可信 Header。
- Schema-per-Tenant 的数据库访问必须在显式事务内使用 `SET LOCAL search_path = ...`，不能使用 session-level `SET search_path`。
- authorization-service 的领域事件必须通过 Outbox Pattern 写入，与本地状态变更处于同一个 PostgreSQL 事务。
- RoleAssignment 热路径允许引入 Caffeine L1 和 Redis L2 缓存；缓存失效必须与角色绑定变更事件对齐。
- 服务间调用后续需要 mTLS 或等价服务身份机制；MVP 阶段可先通过 Port 和测试替身保留边界。
...
## Phase 5: Authorization Runtime Baseline

- 实现 `TenantContextFilter` 和 `TenantContextHolder`。
- 实现 `TenantAwareAuthentication`，承载 `tenantId`、`userId` 和平台角色。
- 实现 `TenantAwareExecutor`，覆盖异步边界的上下文传播。
- 定义 authorization-service 的 `POST /api/v1/authorization/evaluate` 契约。
- 定义 `Role`、`RoleAssignment`、`Policy`、`PolicyRule` 和 `AuthorizationDecision` 模型。
- 实现 Thin Client SDK 的调用边界、超时、熔断和 Fail-Closed 行为。
- 引入授权决策 Outbox 事件模型。
- 编写资源归属校验、DENY 语义、503 Fail-Closed 和跨租户访问测试。
...
- Envoy Gateway 配置改造。
- 下游服务 ABAC 实现。
- 完整 authorization-service PDP 实现。
- OPA/Rego 策略引擎引入。
- 服务拆分为独立 Role、RoleAssignment、Policy 服务。
...
9. 再接入 Kafka 和 Vault。
10. 再实现 Tenant Context Filter 和应用内上下文传播。
11. 再定义 authorization-service 评估 API 与 Thin Client SDK。
12. 再实现 Authorization BC 的 Role、RoleAssignment、Policy 聚合和 Outbox 事件。
...
## Risk 7: 把动态授权放进 Envoy 热路径

如果在 Envoy 层引入 Ext-Authz 并访问数据库，会让每个请求在转发前多一次同步远程调用，增加关键路径延迟。

缓解方式：Envoy 只做 JWT 验证和静态 RBAC；资源级 ABAC 放在 Spring Boot 应用层和 authorization-service 中执行。

## Risk 8: 把租户自定义角色写入 JWT

自定义角色变化频繁，写入 JWT 会导致角色变更在 Token 过期前无法立即生效，也会让 Token 体积随租户复杂度增长。

缓解方式：JWT 只携带平台预定义角色；自定义角色由 authorization-service 查询 RoleAssignment。

## Risk 9: 错误信任客户端 Header

如果下游服务读取客户端直接传入的 `X-Tenant-ID`，攻击者可以伪造租户身份造成 IDOR。

缓解方式：Envoy 必须剥除身份 Header 并重新注入；下游只信任经过网关注入的 Header。

## Risk 10: authorization-service 故障时 Fail-Open

授权服务不可用时放行业务请求会破坏多租户隔离。

缓解方式：Thin Client SDK 默认 Fail-Closed；authorization-service 503 时业务服务向客户端返回 503 并触发告警。

## Risk 11: TenantContext ThreadLocal 泄漏

虚拟线程和线程池场景下，如果未清理或未传播上下文，会出现租户上下文丢失或串租户风险。

缓解方式：Filter 和 Executor 必须使用 `finally` 清理 `TenantContextHolder` 和 `SecurityContextHolder`；异步任务必须通过 `TenantAwareExecutor`。
...
- Authorization BC 是 Core Domain，授权评估逻辑集中在 authorization-service。
- `Role`、`RoleAssignment`、`Policy` 聚合合并部署，不按访问频率拆分服务。
- 租户自定义角色不进入 JWT。
- ABAC 默认 Fail-Closed。
- `denyReason` 使用枚举而不是自由文本。
...
- 测试覆盖每个 Step 的幂等行为。
- PRD 中的授权架构基线可以被拆分为后续 Taskmaster 任务：Tenant Context、authorization-service 契约、Thin Client SDK、Authorization BC 聚合和 Outbox 事件。
- 所有后续任务必须保持 Envoy RBAC 与应用层 ABAC 的职责边界。

## Suggested Taskmaster Task Count

建议 Taskmaster 解析本 PRD 时生成 14 到 18 个顶层任务。任务粒度应围绕领域模型、Ports、Step Pipeline、Provisioning Service、Fake Adapter、状态机、测试、真实 Keycloak Adapter 准备、事件边界、Tenant Context、authorization-service 契约和 Thin Client SDK 拆分。

## References

- `IAL_PRD_身份与访问层需求文档.md`
- `IAL_SAD_身份与访问层架构实施文档.md`
- `services/control-plane/identity-access/.taskmaster/docs/DDD.md`
